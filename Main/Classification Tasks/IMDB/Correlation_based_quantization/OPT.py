"""
Comprehensive Mixed-Precision Quantization Pipeline for OPT Models on IMDB
===========================================================================
...
2. Computing layer sensitivity (calibration + similarity metrics)
3. Clustering layers into precision groups
4. Quantizing and evaluating mixed-precision models

Features:
- Interactive user prompts for model selection and each stage
...
def run_sensitivity_analysis(selected_model, model_config):
    ...
    t0 = time.time()
    layer_outputs = extract_layer_outputs(loader, base_model, device, num_layers=model_config["num_layers"] + 1)
    ...
    for metric, sensitivities in sensitivities_dict.items():
        json_filename = f"layer_sensitivity_{selected_model}_IMDB_{metric}.json"
        json_path = os.path.join("Sensitivities", json_filename)
        with open(json_path, "w") as f:
            json.dump(sensitivities, f, indent=2)
        
        txt_filename = f"layer_sensitivity_{selected_model}_IMDB_{metric}.txt"
        txt_path = os.path.join("Sensitivities", txt_filename)
        with open(txt_path, "w") as f:
            f.write(f"# {metric.upper()} Sensitivities for {selected_model} on IMDB\n")
...
def run_quantization_evaluation(selected_model, model_config):
    ...
    # Load as seq classification model (need to add classifier head)
    try:
        model = AutoModelForSequenceClassification.from_pretrained(model_name).to(device)
    except:
        # If not available, use base model + custom head
    ...
    sens_files = list(sens_dir.glob(f"layer_sensitivity_{selected_model}_IMDB_*.json"))
    
    if not sens_files:
        print(f"❌ No sensitivity files found for {selected_model}. Run Phase 1 first.")
        return
    ...
    - Non-finetuned OPT: use (16/8/4) bits
    - Finetuned OPT: use (32/16/8) bits
    
    4 groups (finer control):
    - Non-finetuned OPT: use (32/16/8/4) bits
    - Finetuned OPT: use (32/16/8/4) bits with top layers at 32
    ...
    log_filename = f"eval_{selected_model}_IMDB_{strategy_name}_{n_clusters}groups_{ts}.txt"
    log_path = os.path.join("Evaluation", log_filename)
    
    with open(log_path, "w") as f:
        f.write(f"# Mixed-Precision PTQ Results | IMDB | {selected_model}\n")
...
def main():
    ...
    print_section("Mixed-Precision Quantization Framework for OPT Models on IMDB")
    print("""
    This framework enables end-to-end mixed-precision quantization with full control:
    
    STEP 0: Model Selection
       - Choose any OPT model variant (125M, 350M, 1.3B, 2.7B, 175B)
    ...
    STEP 2: Quantization & Evaluation
    ...
    while True:
        ...
        if phase_choice == "Exit":
            ...
            run_sensitivity_analysis(selected_model, model_config)
        elif phase_choice == "Phase 2: Quantization & Evaluation":
            run_quantization_evaluation(selected_model, model_config)
