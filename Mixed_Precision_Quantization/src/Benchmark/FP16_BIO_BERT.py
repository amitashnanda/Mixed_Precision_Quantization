...
from transformers import BertTokenizer, BertForSequenceClassification, AutoTokenizer, AutoModelForSequenceClassification, DataCollatorWithPadding
...
# Remove the 'unsupervised' split if it exists
if "unsupervised" in raw_datasets:
    del raw_datasets["unsupervised"]

tokenized_datasets = raw_datasets.map(tokenize_function, batched=True)

# Select the train and test populations
train_population = list(range(len(tokenized_datasets["train"])))
test_population = list(range(len(tokenized_datasets["test"])))

# Select and adjust the datasets
tokenized_datasets["train"] = tokenized_datasets["train"].select(train_population)
tokenized_datasets["test"] = tokenized_datasets["test"].select(test_population)
data_collator = DataCollatorWithPadding(tokenizer=tokenizer)

# Remove unnecessary columns and rename 'label' to 'labels'
...
accuracy_before, _ = train_eval_model(model, train_loader, test_loader, quantized=False)

# Evaluate after quantization
accuracy_after, compression_ratio = train_eval_model(model, train_loader, test_loader, quantized=True)
