...
def main():
    ...
    json_path = os.path.join("Sensitivities", "layer_sensitivity_BERT_IMDB.json")
    with open(json_path, "w") as f:
        json.dump(layer_sensitivities, f, indent=2)

    elapsed = time.time() - t0
    txt_path = os.path.join("Sensitivities", f"layer_sensitivity_BERT_IMDB_runtime_{elapsed:.2f}s.txt")
    with open(txt_path, "w") as f:
        f.write(f"# PWCCA sensitivities for BERT-base-uncased on IMDB\n")
